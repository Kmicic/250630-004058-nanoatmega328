...
void incr(uint32_t* val, int incr, int limit) {
  if (incr < 0 && *val < (uint32_t)(-incr)) *val = 0;
  else *val += incr;
  if (*val > (uint32_t)limit) *val = (uint32_t)limit;
  if (*val < 0) *val = 0;
}
void incr(int8_t* val, int incr, int limit) {
  *val += incr;
...
void setDMY(byte set, int dir); // Forward declaration
...
void settingsSett(int dir) {
  switch (arrowPos) {
    ...
    case 7: incr((int*)&settings.comSensPeriod, (int)dir, 1000); break;
  ...
  switch (arrowPos) {
    ...
    case 4:
      if (curMode == 0) {   // реле
        ...
        incr((int*)&settings.driveTimeout, dir, 30000);
        currentLine = 3;
      }
      break;
    case 5:
      ...
      switch (thisMode) {
        ...
        case 3:
          if (setChannel.sensPeriod < 60) {
            incr((int*)&setChannel.sensPeriod, dir * 2, 32000);
          } else {
            incr((int*)&setChannel.sensPeriod, dir * 60, 32000);
          }
          break;
#if (USE_PID == 1)
    ...
    case 4:
      switch (thisMode) {
        ...
        case 3:
          ...
          incrInt((int*)&setChannel.threshold, dir, 5000);
          currentLine = 3;
          break;
#if (USE_PID == 1)
    ...
    case 5:
      switch (thisMode) {
        ...
        case 3:
          ...
          incrInt((int*)&setChannel.thresholdMax, dir, 5000);
          currentLine = 3;
          break;
#if (USE_PID == 1)
