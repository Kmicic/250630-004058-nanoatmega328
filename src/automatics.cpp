    ...
    if (activeChannel.state                                                             // если канал активен (on/off)
      ...
      switch (activeChannel.mode) {
        ...
        case 2:   // ---------------------- если неделька ----------------------
          ...
          break;
        case 3:   // ---------------------- если датчик ----------------------
          if ((unsigned long)(millis() - timerMillis[curChannel]) >= (unsigned long)(activeChannel.sensPeriod * 1000L)) {
            timerMillis[curChannel] = millis();
...
boolean checkDay(byte channel) {
  ...
  if (bitRead(temp.week, today - 1) &&
      ((temp.weekOff > temp.weekOn) ?
       ((long)thisTime > (long)temp.weekOn && (long)thisTime < (long)temp.weekOff) :
       ((long)thisTime > (long)temp.weekOn || (long)thisTime < (long)temp.weekOff))) return true;
  else return false;
}
